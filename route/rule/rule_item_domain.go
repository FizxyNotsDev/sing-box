...
func (r *DomainItem) Match(metadata *adapter.InboundContext) bool {
	return (metadata.Destination.IsFqdn() && r.matcher.Match(strings.ToLower(metadata.Destination.Fqdn))) ||
		(metadata.SniffHost != "" && r.matcher.Match(strings.ToLower(metadata.SniffHost))) ||
		(metadata.Domain != "" && r.matcher.Match(strings.ToLower(metadata.Domain)))
}

...
func (c *Client) LookupCache(domain string, strategy C.DomainStrategy) ([]netip.Addr, bool, bool) {
	...
	dnsName := dns.Fqdn(domain)
	if strategy == C.DomainStrategyIPv4Only {
		addresses, ttl, err := c.questionCache(dns.Question{
			Name:   dnsName,
			Qtype:  dns.TypeA,
			Qclass: dns.ClassINET,
		}, nil)
		if err != ErrNotCached {
			return addresses, true, c.useStaleCache && ttl == 0
		}
	} else if strategy == C.DomainStrategyIPv6Only {
		addresses, ttl, err := c.questionCache(dns.Question{
			Name:   dnsName,
		...
		if response4 != nil || response6 != nil {
			...
			return sortAddresses(MessageToAddresses(response4), MessageToAddresses(response6), strategy), true, c.useStaleCache && ttl4 == 0
		}
	}
	return nil, false, false
}
